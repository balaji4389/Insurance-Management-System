...
namespace insurancenew
{
	...
	/// </summary>
	public partial class cust_pwd_change : System.Web.UI.Page
	{
       ...
       SqlConnection con=new SqlConnection("Data Source=BALAJI\\SQLEXPRESS;Initial Catalog=Insurance_Management_System;Integrated Security=True") ;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				txt_agent_id.Text=Session["customer_id"].ToString();
...
//save
		...
		{
            cmd = new SqlCommand("select cust_pwd_new from customer_security_master where cust_id=" + Session["customer_id"] + "", con);
			con.Open();
			...
			if(txt_pwd_old.Text==a)
			{
				...
				cmd=new SqlCommand("update customer_security_master set cust_pwd_old='"+str+"',cust_pwd_new='"+txt_pwd_new.Text+"',cust_pwd_change_date='"+txt_date.Text+"' where cust_id="+txt_agent_id.Text+"",con);
				cmd.ExecuteNonQuery();
				con.Close();
